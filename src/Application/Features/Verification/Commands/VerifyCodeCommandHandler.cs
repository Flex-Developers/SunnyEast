    ...
    : IRequestHandler<VerifyCodeCommand, VerifyResponse>
{
    public async Task<VerifyResponse> Handle(VerifyCodeCommand request, CancellationToken ct)
    {
        ...
        switch (s.Purpose)
        {
            ...
            case "link":
                // просто помечаем как подтверждённую; контакт подключит AccountController
                var linked = s with { IsVerified = true, Code = string.Empty };
                await store.UpdateAsync(linked, ct);
                break;
